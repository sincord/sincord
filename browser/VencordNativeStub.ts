...
window.VencordNative = {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Sincord/Sincord" }),
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("SincordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("SincordSettings", JSON.stringify(s)),
