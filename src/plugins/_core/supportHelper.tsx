...
import { isAnyPluginDev, isSincordGuild, isSincordSupport, isSupportChannel, tryOrElse } from "@utils/misc";
...
const TrustedRolesIds = [
    ...
    SINCORD_TEAM, // Sincord Team
    DONOR_ROLE_ID, // Sincord Donor
    CONTRIB_ROLE_ID, // Sincord Contributor
...
async function generateDebugInfoMessage() {
    ...
    clientString += `${clientInfo.shortHash ? ` • [${clientInfo.shortHash}](<https://github.com/Sincord/Sinbop/commit/${clientInfo.hash}>)` : ""}`;
    ...
    const info = {
        Sincord:
            `v${VERSION} • [${gitHashShort}](<https://github.com/Sincord/Sincord/commit/${gitHash}>)` +
    ...
    const commonIssues = {
        ...
        "Sincord DevBuild": !IS_STANDALONE,
...
function DevBuildConfirmModal(props: RenderModalProps) {
    ...
    return (
        ...
        >
            <div>
                <Paragraph>You are using a custom build of Sincord, which we do not provide support for!</Paragraph>
...
export default definePlugin({
    ...
    commands: [
        {
            name: "sincord-debug",
            description: "Send Sincord debug info",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isSincordGuild(ctx?.guild?.id, true),
            execute: async () => ({ content: await generateDebugInfoMessage() })
        },
        {
            name: "sincord-plugins",
            description: "Send Sincord plugin list",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isSincordGuild(ctx?.guild?.id, true),
            execute: async () => {
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    openModal(props => (
                        ...
                        >
                            <div>
                                <Paragraph>You are using an outdated version of Sincord! Chances are, your issue is already fixed.</Paragraph>
                                <Paragraph className={Margins.top8}>
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                openModal(props => (
                    ...
                    >
                        <div>
                            <Paragraph>You are using an externally updated Sincord version, which we do not provide support for!</Paragraph>
                            <Paragraph className={Margins.top8}>
                                Please either switch to an <Link href="https://sincord.org/download">officially supported version of Sincord</Link>, or
    ...
    renderMessageAccessory(props) {
        ...
        const sincordSupport = isSincordSupport(props.message.author.id);
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message Sincord & Vencord plugin developers for support!
                <br />
